...
"""
@Author: Bing Tang
@Time: “2022/11/10 14:58”
通过控制 功率 来 控制 信噪比 底噪 为 1
"""
import os
import argparse
import time
...
import numpy as np
from utils import  SNR_to_noise, initNetParams, control_power_semantic_block_train_step, SeqtoText, ControlPower_train_mi, DENSE
...
parser.add_argument('--checkpoint_path', default='./checkpoints/Train_SemanticBlock_Relay_PowerControl', type=str)
...
def train(epoch, args, net1, mi_net=None):
    ...
    for sents in pbar:
        ...
        noise_std = 1 / math.sqrt(2)
        if mi_net is not None:
            # mi = train_mi(net, mi_net, sents, noise_std[0], pad_idx, mi_opt, args.channel)
            mi = ControlPower_train_mi(net1, mi_net, sents, noise_std, pad_idx, mi_opt, args.channel, SNR)
            loss, los_cos = control_power_semantic_block_train_step(net1, sents, sents, noise_std, pad_idx, optimizer, criterion, args.channel, start_idx,
                                                      sentence_model, StoT, SNR, mi_net)
        ...
        if total_loss < std_acc:
            ...
            if epoch % 10 == 0:
                torch.save({
                    ...
                    'epoch': epoch,
                }, args.checkpoint_path + '/1110DeepTest_net_checkpoint.pth')
            std_acc = total_loss
            #1031DeepTest_net_checkpoint.pth 0-18db，之前的是4-10db
            #1110 路损 是 （100/160） 的 -2  选择 SNR = 6dB
